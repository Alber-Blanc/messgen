...
def render_protocol(pkg: str, proto_name: str, proto_def: Protocol, types: dict):
    ...
    yield ")"
    yield ""
    yield f"func (mid {proto_name}_MsgId) String() string {{"
    yield f"\tswitch (mid) {{"
    for id, msg in proto_def.messages.items():
        yield f"\t\tcase {proto_name}_{toGoName(msg.name)}_Id:"
        yield f"\t\t\treturn \"{toGoName(msg.name)}\""
    yield f"\t}}"
    yield f"\treturn \"Unknown message\""
    yield f"}}"
    yield ""
    yield f"type {proto_name}Message interface {{"
    yield f"\tMsgID() {proto_name}_MsgId"
    yield f"\tProtoID() uint8"
    yield f"\tHash() uint64"
    yield f"\tData() {proto_name}MessageData"
    yield f"}}"
    yield f""
    yield f"type {proto_name}MessageData interface {{"
    yield f"\tSerialize(output []byte) (uint32, error)"
    yield f"\tDeserialize(input []byte) (uint32, error)"
    yield f"\tSerializedSize() uint32"
    yield f"}}"
    yield f""
    for id, msg in proto_def.messages.items():
        tp = types[msg.type]
        ...
        yield f"}}"
        yield f""
        yield f"func (m *{proto_name}_{toGoName(msg.name)}) MsgID() {proto_name}_MsgId {{"
        yield f"\treturn {proto_name}_{toGoName(msg.name)}_Id"
        yield f"}}"
        yield f""
        yield f"func (m *{proto_name}_{toGoName(msg.name)}) ProtoID() uint8 {{"
        yield f"\treturn {proto_name}_Id"
        yield f"}}"
        yield f""
        ...
        yield f"}}"
        yield f""
        yield f"func (m *{proto_name}_{toGoName(msg.name)}) Data() {proto_name}MessageData {{"
        yield f"\treturn &m.Msg"
        yield f"}}"
        yield f""
    ...
    yield f"\tmsg := P(new(T))"
    yield f""
    yield f"if msg.ProtoID() != {proto_name}_Id || msg.MsgID() != mid {{"
    yield f"\treturn fmt.Errorf(\"Message id doesn't match for the message {proto_name}_%s: %d != %d \", mid.String(), msg.MsgID(), mid)"
    yield f"}}"
    yield f""
    yield f"\tsz, err := msg.Data().Deserialize(body)"
    yield f"\tif err != nil {{"
    yield f"\t\treturn fmt.Errorf(\"Failed to read message {proto_name}_%s: %s\", mid, err)"
    yield f"\t}} else if int(sz) != len(body) {{"
    yield f"\t\treturn fmt.Errorf(\"Readed size isn't valid for the message {proto_name}_%s: %d != %d\", mid, sz, len(body))"
    yield f"\t}}"
    yield f"\t"
    yield f"\terr = process(msg)"
    yield f"\tif err != nil {{"
    yield f"\t\treturn fmt.Errorf(\"Failed to handle message {proto_name}_%s: %s\", mid,  err)"
    yield f"\t}}"
    yield f""
    yield f"\treturn nil"
    yield f"\t}}"
    yield f"}}"
    yield f""
    yield f"func (handlers {proto_name}Dispatcher) Dispatch(mid {proto_name}_MsgId, body []byte) error {{"
    yield f"\thandle := handlers[int(mid)]"
